...
const period_login = async (res) => {
  ...
  try {
    ...
    const dateLogin = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.sql.database}.dbo.FA_Period_Time_Login @BranchID, @RoundID`);
    //sql.close()
...
const getsperiod_round = async (selectQuery) => {
  ...
  try {
    ...
    const allround_period = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.sql.database}.dbo.FA_Period_all_rounds @BranchID, @depCode, @personID`);
    //sql.close()
...
const craete_period = async (res) => {
  ...
  try {
    ...
    const fa_create_period = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.sql.database}.dbo.[FA_Create_Assets_Counted_After_Period] @begindate ,@enddate ,@branchid ,@description ,@usercode, @depcode, @personID, @keyID`);
    //sql.close()
...
const FA_Control_Fetch_Branch_Period = async (branch_period) => {
  ...
  try {
    ...
    const auto_DeapartMent = await pool.request()
      ...
      .query(`exec ${config.PTEC.object_test_ops.sql.database}.dbo.FA_Control_Fetch_Branch_Period @usercode`);
    //sql.close()
