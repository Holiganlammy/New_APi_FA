...
const Districts_List = async (req) => {
  ...
  const addOwner = await pool.request().query(`exec [TEST_USERSRIGHT].dbo.Districts_List`);
...
const Amphures_List = async (req) => {
  ...
  let pool = await sql.connect(config.PTEC.objcn_usersright.sql);
  //const sqlOueries = await utils.loadSqlOueries("TEST_OPS");
  const addOwner = await pool.request().query(`exec [TEST_USERSRIGHT].dbo.Amphures_List`);
...
const Provinces_List = async (req) => {
  ...
  let pool = await sql.connect(config.PTEC.objcn_usersright.sql);
  //const sqlOueries = await utils.loadSqlOueries("TEST_OPS");
  const addOwner = await pool.request().query(`exec [TEST_USERSRIGHT].dbo.Provinces_List`);
  //sql.close()
  return addOwner.recordset;
};
